...
bp_spreads %>%
  filter(portfolio %in% c(1,2,3,4,5)) %>%
...
portfolio_returns %>%
  group_by(portfolio) %>%
  summarise(
    `Mean Return (%)` = mean(ret_ew, na.rm = TRUE) * 12 * 100,
    `SD (%)` = sd(ret_ew, na.rm = TRUE) * sqrt(12) * 100,
    `Sharpe Ratio` = (`Mean Return (%)`) / (`SD (%)`)
  ) %>%
  mutate(across(where(is.numeric), round, 2)) %>%
  kable(
    caption = "Portfolio Performance Summary (Equal-Weighted Returns)",
    align = "c"
  ) %>%
  kable_styling(
